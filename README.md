...
資格取得が一服し、そろそろ、身につけたり、日頃の情報収集で見聞きした技術について棚卸しと素振りがしたくなったのと、自分の学んだり体験したことをアウトプットする場所がちょうどなかったので、まずはブログからつくろうかなと思いました

### 使用技術

フロントエンドに TypeScript/Next.js/Apollo Client/S3(静的ホスティング)/Cloud Front を, API サーバーに TypeScript/Apollo Server/Cloud Run を, 記事入稿システムに TypeScript/Python/Lambda/SQS/DynamoDB を, AWS リソースの管理(IaC)に AWS CDK/TypeScript, CI/CD に GitHub Actions を使用しています。

### 工夫した点

元々、自分の興味分野がフロントエンドなので、最近気になっていた TypeScript/Next.js(React)/GraphQL のような流行りの技術を使用したスタック構成でプロダクトを作り、フロントエンド技術をキャッチアップすることを意図していました。

また、 CI/CD 環境を整えて、いかにストレスなく開発や記事の執筆ができるようにできるか、ということも意識しています。コードや記事を書くことに全力を使い切った後、疲れた頭で複雑な手順のデプロイを行うのは、事故もまねきやすくストレスなので避けたいなという考えです。

また、Next.js を用いた JAM スタック構成のブログでは、アプリソースと一緒に記事データをリポジトリ管理する手法をよく見かけるのに対して、自分のプロダクトでは記事データは DynamoDB による管理としていますが、これは、記事に対して、より多くのメタデータ(閲覧数や順位など)を持たせるためと、サーバーサイドで情報をまとめて編集したり、またサーバーサイドで処理した情報(シンタックスハイライト後の .html コンテンツなど)を書き込みたかった、などの要件から、このような管理方法にしています。

フロントエンドと記事入稿システムを AWS 上にまとめているのに対して、API サーバーだけ GCP の Cloud Run で動かしていてやや浮いているのですが、これは 1. すぐに使えるパブリック IP アドレス(エンドポイント)が払い出されて 2. デプロイが簡単で 3. 料金が安い の観点で使えそうなホスティングサービスを探したなかで、一番マッチしていたからです。Beanstalk は EC2 インスタンスをホストするので料金が高く、 Lambda はパブリックアクセスに API Gaeteway などの他のリソースが必要で、ECS Fargate は 0 スケールからの起動が遅く、どれも要件にマッチしませんでした。GCP Cloud Run であれば、HTTPS のアドレスが自動で付与される上、特別な設定無しで 0 スケールし、コールドスタートからの起動も数秒以内と早く、デプロイもコンソールを数クリック設定した後、リポジトリに dockerfile を置いておけば、対象ブランチの更新時に自動で行っくれるため、この上となく要件とマッチしています。
