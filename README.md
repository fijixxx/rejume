...
資格取得が一服し、そろそろ身につけたり、日頃の情報収集で見聞きした技術について棚卸しと素振りがしたくなったのと、これから新たに自分の学んだり体験したことをアウトプットする場所がちょうどなかったので、まずはブログからつくろうかなと思いました
...
環境構成については、以下に図を作成しました。
https://github.com/fijixxx/sublog-architecture

### 工夫した点

自分の興味分野がフロントエンドなので、最近気になっていた TypeScript/Next.js(React)/GraphQL のような流行りの技術を使用したスタック構成でプロダクトを作り、フロントエンド技術をキャッチアップすることを意図しました。
...
フロントエンドと記事入稿システムを AWS 上にまとめているのに対して、API サーバーだけ GCP の Cloud Run で動かしていてやや浮いているのですが、これは 1. すぐに使えるパブリック IP アドレス(エンドポイント)が払い出されて 2. デプロイが簡単で 3. 料金が安い の観点でホスティングサービスを探したなかで、 GCP Cloud Run が 1 番マッチしていたからです。GCP Cloud Run は、デプロイすると HTTPS のパブリックエンドポイントが自動で付与される上、特別な設定無しで 0 スケールし、コールドスタートからの起動も数秒以内と早く、デプロイ設定もコンソールを数クリックした後、リポジトリに dockerfile を置いておけば、対象ブランチの更新時に自動で行ってくれるため、この上となく要件とマッチしていると感じています。
